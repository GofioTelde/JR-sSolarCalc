...
function PanelTiltDiagram({
  tiltDeg,
  panelHeightAlongSlopeM,
  bottomEdgeM,
  orientation,
}: TiltDiagramProps) {
  ...
  return (
    ...
    >
      ...
      <text
        x={((bx + tx) / 2).toFixed(1)}
        y={(groundY + padBot * 0.65).toFixed(1)}
        textAnchor="middle"
        fontSize={9}
        fill="#6b7280"
      >
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex justify-between items-start py-1.5 border-b border-gray-100 dark:border-gray-700 last:border-0">
      ...
      <span className="font-medium text-gray-800 dark:text-white text-sm text-right max-w-[55%] break-words">
        {value}
      </span>
    </div>
  );
...
function Block({
  ...
  return (
    <div
      className={`border ${border} rounded-xl overflow-hidden mb-5 print:mb-4 print:break-inside-avoid`}
    >
      <div
        className={`${header} px-4 py-2.5 font-bold text-gray-800 dark:text-white text-sm`}
      >
        {title}
      </div>
...
const Phase5Summary: React.FC<Props> = ({ onReset }) => {
  ...
  const {
    ...
    selectedComponents,
    systemType,
    panelBottomEdgeHeightM,
  } = data;

  const needsBatteries =
    (hasBatteries ?? true) || installationType === "off-grid";

  const panel = selectedComponents?.panelId
    ? byId(selectedComponents.panelId)
    : null;
  const battery = selectedComponents?.batteryId
    ? byId(selectedComponents.batteryId)
    : null;
  const inverter = selectedComponents?.inverterId
    ? byId(selectedComponents.inverterId)
    : null;
  const mppt = selectedComponents?.mpptId
    ? byId(selectedComponents.mpptId)
    : null;

  const modalityInfo = installationModality
    ? MODALITY_INFO[installationModality]
    : null;
  const installNotes = installationModality
    ? (MODALITY_INSTALL_NOTES[installationModality] ?? [])
    : [];
  ...
  const protecciones = React.useMemo(() => {
    ...
    const fuse = pick<Proteccion>(
      "fusible_dc",
      (p) => (p.amperaje ?? 0) >= fuseA && (p.tension_max ?? 0) >= vocString,
    );
    if (fuse)
      items.push({
        item: fuse,
        qty: numStrings * 2,
        unit: "uds.",
        nota: `${numStrings} strings × 2 polos`,
      });

    // DC seccionador
    const sec = pick<Proteccion>(
      "seccionador_dc",
      (p) => (p.amperaje ?? 0) >= secA && (p.tension_max ?? 0) >= vocString,
    );
    if (sec)
      items.push({ item: sec, qty: 1, unit: "ud.", nota: "Cuadro DC paneles" });

    // SPD DC
    const spdDc = pick<Proteccion>(
      "spd_dc",
      (p) => (p.tension_max ?? 0) >= vocString,
    );
    if (spdDc)
      items.push({
        item: spdDc,
        qty: 1,
        unit: "ud.",
        nota: "Protección rayos DC",
      });

    // AC magnetotérmico
    const magA = Math.ceil((inverterAcA * 1.25) / 5) * 5;
    const mag = pick<Proteccion>(
      "magnetotermico_ac",
      (p) =>
        (p.amperaje ?? 0) >= magA && (p.polos ?? 1) === (isTrifasico ? 3 : 1),
    );
    if (mag)
      items.push({
        item: mag,
        qty: 1,
        unit: "ud.",
        nota: `${inverterKw.toFixed(1)} kW → ${magA}A`,
      });

    // AC diferencial
    const dif = pick<Proteccion>(
      "diferencial_ac",
      (p) =>
        (p.amperaje ?? 0) >= magA && (p.polos ?? 2) === (isTrifasico ? 4 : 2),
    );
    if (dif)
      items.push({ item: dif, qty: 1, unit: "ud.", nota: "30mA Tipo A" });

    // SPD AC
    const spdAc = pick<Proteccion>(
      "spd_ac",
      (p) => (p.polos ?? 1) === (isTrifasico ? 3 : 1),
    );
    if (spdAc)
      items.push({
        item: spdAc,
        qty: 1,
        unit: "ud.",
        nota: "Protección rayos AC",
      });

    // Cable DC 6mm² (always a safe default)
    const cabDc = ALL_PROT.find(
      (p) => p.tipo === "cable_dc" && (p as Proteccion).seccion_mm2 === 6,
    ) as Proteccion | undefined;
    if (cabDc)
      items.push({
        item: cabDc,
        qty: 1,
        unit: "rollo 100m",
        nota: "Positivo + negativo DC",
      });

    // Cable AC
    const cabAc = pick<Proteccion>(
      "cable_ac",
      (p) =>
        (p.corriente_max ?? 0) >= inverterAcA &&
        (p.fases ?? 1) === (isTrifasico ? 3 : 1),
    );
    if (cabAc)
      items.push({ item: cabAc, qty: 1, unit: "rollo 50m", nota: "Cuadro AC" });

    // Monitor
    const mon = ALL_PROT.find(
      (p) => p.tipo === "monitorizacion" && !(p as Proteccion).display,
    ) as Proteccion | undefined;
    if (mon) items.push({ item: mon, qty: 1, unit: "ud.", nota: "App móvil" });

    // Puesta a tierra
    const cabTie = ALL_PROT.find(
      (p) => p.tipo === "puesta_tierra" && (p as Proteccion).seccion_mm2 === 16,
    ) as Proteccion | undefined;
    if (cabTie)
      items.push({
        item: cabTie,
        qty: 1,
        unit: "rollo 50m",
        nota: "Tierra estructura + equipos",
      });
    const pica = ALL_PROT.find(
      (p) => p.tipo === "puesta_tierra" && (p as Proteccion).longitud_m,
    ) as Proteccion | undefined;
    if (pica)
      items.push({
        item: pica,
  ...
  const systemTypeLabel: Record<string, string> = {
    hibrido: "Híbrido",
    separados: "Componentes separados",
  };
    ...
    <div className="max-w-lg mx-auto px-2 py-2 print:max-w-full print:px-6 print:py-4">
      ...
      <div className="hidden print:block text-center mb-6 pb-4 border-b-2 border-gray-300">
        <h1 className="text-2xl font-bold">
          🌞 JR's SolarCalc — Lista de Compra
        </h1>
      ...
      <Block title="📋 Resumen de la instalación" color="blue">
        {modalityInfo && (
          <div
            className={`flex items-center gap-2 mb-3 px-3 py-2 rounded-lg ${modalityInfo.bgClass} ${modalityInfo.textClass} text-sm font-medium`}
          >
        ...
        {location?.locationName && (
          <Row label="Ubicación" value={location.locationName} />
        )}
        {location && (
          <Row
            label="Coordenadas"
        ...
        <Row
          ...
          value={`${consumption?.monthlyKWh ?? "—"} kWh/mes`}
        />
        <Row
          label="Consumo diario"
          value={
            consumption
              ? `${(consumption.monthlyKWh / 30).toFixed(1)} kWh/día`
              : "—"
          }
        />
        ...
        <Row
          label="Tipo de panel"
          value={panelType === "monofacial" ? "Monofacial" : "Bifacial"}
        />
        <Row
          label="Sistema"
          value={systemTypeLabel[systemType ?? "hibrido"] ?? "—"}
        />
        {solarCalc && (
          <>
            ...
            <Row
              ...
              value={`${(solarCalc.requiredPowerWp / 1000).toFixed(2)} kWp`}
            />
            <Row
              label="Generación anual est."
              value={`${solarCalc.annualGenerationKWh.toFixed(0)} kWh/año`}
            />
          </>
        )}
      </Block>
      ...
      <Block title="🛒 Lista de componentes a comprar" color="emerald">
        <div className="space-y-4">
          ...
          {panel && selectedComponents && (
            <div className="flex items-start gap-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  {panel.potencia_pmax as number} Wp ·{" "}
                  {panel.tecnologia as string} · {panel.eficiencia as number}%
                  efic.
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  {panel.ancho_mm as number} × {panel.alto_mm as number} mm ·{" "}
                  {panel.peso_kg as number} kg
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  Garantía: {panel.garantia_producto as number}a producto /{" "}
                  {panel.garantia_potencia as number}a potencia
                </div>
              </div>
              <div className="flex-shrink-0 text-right">
                <div className="text-xl font-bold text-green-600 dark:text-green-400">
                  ×{selectedComponents.numPanels}
                </div>
                <div className="text-[10px] text-gray-400">unidades</div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  {(
                    (panel.superficie_m2 as number) *
                    selectedComponents.numPanels
                  ).toFixed(1)}{" "}
                  m²
                </div>
          ...
          {needsBatteries &&
            ...
            (() => {
              const isModularTow =
                (battery.tipo as string) === "lifepo4_hv" &&
                (battery.tension_nominal as number) < 200;
              ...
              const unitLabel = isModularTow
                ? `${totalMods} mód.`
                : `×${totalMods}`;
              const subLabel = isModularTow
                ? `${towers} BMS · ${totalKwh} kWh`
                : "unidades";
              return (
                <div className="flex items-start gap-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800">
                  <span className="text-2xl flex-shrink-0">🔋</span>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      {kwh_ud} kWh útiles · {battery.tension_nominal as number}{" "}
                      V
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      {(battery.ciclos_vida_80dod as number).toLocaleString()}{" "}
                      ciclos · {battery.eficiencia_carga_descarga as number}%
                      efic.
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      Garantía: {battery.garantia as number} años
                    </div>
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <div className="text-xl font-bold text-green-600 dark:text-green-400">
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
              ...
              <div className="flex-1 min-w-0">
                ...
                {(inverter.potencia_ac_nominal as number) > 0 && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    {((inverter.potencia_ac_nominal as number) / 1000).toFixed(
                      1,
                    )}{" "}
                    kW AC
                    {(inverter.mppt_numero as number) > 0 &&
                      ` · ${inverter.mppt_numero as number} MPPTs`}
                  </div>
                )}
                {(inverter.potencia_nominal as number) > 0 && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    {((inverter.potencia_nominal as number) / 1000).toFixed(1)}{" "}
                    kW · {inverter.forma_onda as string}
                  </div>
                )}
                {(inverter.eficiencia_max as number) > 0 && (
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {inverter.eficiencia_max as number}% efic. máx.
                  </div>
                )}
              </div>
              <div className="flex-shrink-0 text-right">
          ...
          {mppt && (
            <div className="flex items-start gap-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  {mppt.corriente_max_salida as number}A · Vdc máx.{" "}
                  {mppt.tension_entrada_max as number}V
                </div>
              </div>
              <div className="flex-shrink-0 text-right">
      ...
      {selectedComponents &&
        (() => {
          ...
          const EXPLAIN: Record<string, string> = {
            ...
            battery:
              "Almacenan la energía sobrante para usarla de noche o en días nublados.",
            mppt: "Regula la carga de las baterías desde los paneles y maximiza la energía captada.",
            inverter_hibrido:
              "Cerebro de la instalación: convierte DC en AC 230V, gestiona baterías y red eléctrica.",
            inverter_hibrido_isla:
              "Cerebro de la instalación aislada: convierte DC en AC 230V y gestiona baterías en modo isla, sin necesidad de red eléctrica.",
            inverter_offgrid:
              "Convierte la corriente DC de las baterías en AC 230V para alimentar tus electrodomésticos.",
            inverter_red:
              "Convierte la corriente DC de los paneles en AC 230V y vuelca la producción solar a la red eléctrica.",
          };
          ...
          const invObj = inverter as {
            precio_ud?: number;
            nombre?: string;
          } | null;
          const mpptObj = mppt as {
            precio_ud?: number;
            nombre?: string;
          } | null;
          ...
          if (panelObj) {
            ...
            lines.push({
              label: panelObj.nombre,
              explain: EXPLAIN.panel,
              qty,
              unit: "uds.",
              precioUd: pud,
              total: pud ? pud * qty : null,
            });
          }
          if (mpptObj) {
            ...
            lines.push({
              label:
                (mpptObj as { nombre?: string }).nombre ?? "Controlador MPPT",
              explain: EXPLAIN.mppt,
              qty: 1,
              unit: "ud.",
              precioUd: pud,
              total: pud,
          ...
          if (needsBatteries && battObj) {
            ...
            const isModularTower =
              isHVBatt && (battObj.tension_nominal as number) < 200;
            if (isModularTower) {
              const seriesPerTower =
                selectedComponents.batterySeriesPerTower ?? totalModules;
              const towers = selectedComponents.batteryTowersCount ?? 1;
              ...
              lines.push({
                label: battObj.nombre,
                explain: kwhExplain,
                qty: totalModules,
                unit: "mód.",
                precioUd: pud,
                total: pud ? pud * totalModules : null,
              });
            } else {
              const totalKwh = (kwh_ud * totalModules).toFixed(1);
              const hvTag = isHVBatt
                ? ` · ${battObj.tension_nominal as number}V HV`
                : "";
          ...
          if (invObj) {
            ...
            lines.push({
              label: (invObj as { nombre?: string }).nombre ?? "Inversor",
              explain: EXPLAIN[explainKey],
              qty: 1,
              unit: "ud.",
              precioUd: pud,
              total: pud,
          ...
          return (
            <Block title="🛒 Lista de compra completa" color="purple">
              ...
              <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl text-xs text-amber-800 dark:text-amber-300">
                ⚠️ <strong>Precios estimados</strong> — valores orientativos de
                mercado (sin IVA, IGIC). Los precios reales varían según
                proveedor, marca y fecha. Solicita presupuesto a un instalador
                certificado.
              </div>
                  ...
                  >
                    <div className="flex justify-between items-start gap-2">
                      <div className="min-w-0 flex-1 overflow-hidden">
                        <p className="text-sm font-semibold text-gray-800 dark:text-white leading-tight truncate">
                          {l.label}
                        </p>
                        <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5 break-words">
                          {l.explain}
                        </p>
                      </div>
                      <div className="flex-shrink-0 text-right ml-2">
                        ...
                        {l.qty > 1 && l.precioUd && (
                          <p className="text-[10px] text-gray-400 font-normal whitespace-nowrap">
                            ({l.precioUd} €/ud)
                          </p>
                        )}
                      </div>
              ...
              {needsBatteries &&
                battObj &&
                (battObj.tipo as string) === "lifepo4_hv" &&
                (battObj.tension_nominal as number) >= 200 &&
                (selectedComponents.numBatteries ?? 0) > 2 &&
                (() => {
                  ...
                  return (
                    <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-xl text-xs">
                      <p className="font-bold text-amber-800 dark:text-amber-300 mb-1">
                        ⚠️ Batería HV {voltaje}V — {qty} unidades ({totalKwh}{" "}
                        kWh)
                      </p>
                      <p className="text-amber-700 dark:text-amber-400 mb-2">
                        Las baterías HV de alta capacidad integran electrónica
                        de potencia interna, lo que las hace más caras por kWh
                        que las LV (48V). Con {qty} unidades el coste es
                        elevado.
                        {pud
                          ? ` Precio/ud: ${pud.toLocaleString("es-ES")} €.`
                          : ""}
                      </p>
                      <p className="text-amber-600 dark:text-amber-500">
                        💡 Si tu inversor lo permite, considera cambiar a
                        sistema LV (48V): baterías Pylontech US5000 a ~1.300
                        €/ud ofrecen 4,77 kWh cada una, con un coste total muy
                        inferior para la misma capacidad.
                      </p>
                    </div>
                  );
                })()}
              ...
              {protLines.length > 0 && (
                <>
                  <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">
                    Protecciones y cableado (obligatorios)
                  </p>
                  <div className="space-y-1 mb-4">
                    {protLines.map((l, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-start py-1.5 border-b border-gray-100 dark:border-gray-700 last:border-0 gap-2"
                      >
              ...
              <div className="space-y-1 mb-4">
                <div className="flex justify-between gap-2 py-1.5 border-b border-gray-100 dark:border-gray-700 text-xs">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400">
                      Raíles, tornillería y anclajes para fijar los paneles al
                      tejado o suelo
                    </p>
                  </div>
                  <p className="text-purple-600 dark:text-purple-400 font-medium flex-shrink-0 whitespace-nowrap">
                    ~{estructuraCost.toLocaleString("es-ES")} €
                  </p>
                </div>
                <div className="flex justify-between gap-2 py-1.5 text-xs">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400">
                      Instalador certificado (obligatorio para legalización)
                    </p>
                  </div>
                  <p className="text-purple-600 dark:text-purple-400 font-medium flex-shrink-0 whitespace-nowrap">
                    ~{manoCost.toLocaleString("es-ES")} €
                  </p>
                </div>
              </div>
      ...
      {location?.magneticSouth && (
        <Block title="🧭 Orientación de los paneles" color="orange">
          <div className="text-sm text-gray-700 dark:text-gray-300 mb-3 p-2 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
            Orienta los paneles hacia el <strong>sur geográfico</strong> usando
            el azimut magnético calculado.
          </div>
          <Row
            label="Azimut magnético sur"
            value={`${location.magneticSouth.toFixed(1)}°`}
          />
          {location.magneticDeclination !== undefined && (
            <Row
              label="Declinación magnética"
              value={`${location.magneticDeclination.toFixed(2)}° ${location.declinationDirection ?? ""}`}
            />
          )}
          <Row
            label="Sur geográfico"
            value={`${location.geographicSouth ?? 180}°`}
          />
      ...
      {selectedComponents &&
        ...
        (() => {
          ...
          const bottomEdge = panelBottomEdgeHeightM ?? 0.3;
          const tilt =
            solarCalc?.optimalTiltDeg ??
            Math.round((location?.latitude ?? 37) * 0.9);
          const tiltRad = (tilt * Math.PI) / 180;
          const topEdgePortrait =
            bottomEdge + heightPortraitM * Math.sin(tiltRad);
          const topEdgeLandscape =
            bottomEdge + heightLandscapeM * Math.sin(tiltRad);
          ...
          const pitchPortrait =
            heightPortraitM *
            (Math.cos(tiltRad) + Math.sin(tiltRad) / Math.tan(elevRad));
          const pitchLandscape =
            heightLandscapeM *
            (Math.cos(tiltRad) + Math.sin(tiltRad) / Math.tan(elevRad));
          const gapPortrait = +(
            pitchPortrait -
            heightPortraitM * Math.cos(tiltRad)
          ).toFixed(2);
          const gapLandscape = +(
            pitchLandscape -
            heightLandscapeM * Math.cos(tiltRad)
          ).toFixed(2);
          ...
          return (
            <Block title="📐 Diagrama de inclinación del panel" color="purple">
              ...
              <p className="mt-2 text-[10px] text-gray-400 dark:text-gray-500 leading-relaxed">
                📏 Paso mínimo calculado sin sombras de 10h–14h · solsticio de
                invierno (21 dic) · lat. {lat.toFixed(1)}° · elev. solar ref.{" "}
                {elevDeg.toFixed(1)}°
              </p>
            </Block>
          );
      ...
      {selectedComponents?.stringConfig && (
        <Block title="🔌 Configuración de string" color="blue">
          <Row
            label="Paneles en serie por string"
            value={`${selectedComponents.stringConfig.panelsInSeries}`}
          />
          <Row
            label="Strings por MPPT"
            value={`${selectedComponents.stringConfig.stringsPerMppt}`}
          />
          <Row
            label="Tensión Vmp del string"
            value={`${selectedComponents.stringConfig.stringVoltageVmp.toFixed(0)} V`}
          />
          <Row
            label="Tensión Voc del string"
            value={`${selectedComponents.stringConfig.stringVoltageVoc.toFixed(0)} V`}
          />
          <Row
            label="Total paneles configuración"
            value={`${selectedComponents.stringConfig.totalPanels} uds.`}
          />
      ...
      <div className="mt-6 flex flex-col gap-3 pt-5 border-t border-gray-200 dark:border-gray-700 print:hidden">
        ...
        <button
          onClick={() => {
            const lines = [
              ...
              location?.locationName
                ? `Ubicación: ${location.locationName}`
                : "",
              ...
              "COMPONENTES:",
              panel && selectedComponents
                ? `- ${panel.nombre} ×${selectedComponents.numPanels}`
                : "",
              needsBatteries && battery && selectedComponents
                ? `- ${battery.nombre} ×${selectedComponents.numBatteries}`
                : "",
              inverter ? `- ${inverter.nombre} ×1` : "",
              mppt ? `- ${mppt.nombre} ×1` : "",
              "",
              costEst
                ? `TOTAL ESTIMADO: ${costEst.totalCostEur.toLocaleString("es-ES")} €`
                : "",
            ]
              .filter(Boolean)
              .join("\n");
        ...
        <button
          onClick={() => {
            if (
              window.confirm(
                "¿Seguro que quieres empezar de nuevo? Se borrarán todos los datos.",
              )
            ) {
              reset();
              onReset();
